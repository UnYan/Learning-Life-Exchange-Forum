...
public interface NoticesRepository extends JpaRepository<Notices,Integer> {
    @Query(value="select * from notices where toid=?1 and flag=0 order by id DESC ", nativeQuery=true)
    List<Notices> findLike(Integer id);
    @Query(value="select * from notices where toid=?1 and flag=1 order by id DESC ", nativeQuery=true)
